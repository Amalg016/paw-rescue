...
export default function StaffDogList() {
    ...
    useEffect(() => {
        ...
        const fetchDogs = async () => {
            try {
                const response = await devAPI.get(`/dogs`);
                const approvalsResponse = await devAPI.get(`/approvals/${id}`);
    ...
    const handleAdopt = async (currentDog, index, e) => {
        ...
        try {
            const response = await devAPI.post(`/approvals`, { id, dogId: currentDog.id });
